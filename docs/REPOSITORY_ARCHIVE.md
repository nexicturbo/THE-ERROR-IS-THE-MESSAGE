...
The default per-file budget is **90 MiB**, below GitHub's normal 100 MiB file limit, and the default per-run budget is **1 GiB**. Oversized, missing, inaccessible, truncated or login-page responses produce a failed manifest entry and nonzero exit, not silent success. The CLI supports `--max-asset-bytes` and `--max-total-bytes`, but larger files need an appropriate separate storage/LFS policy before committing to GitHub. Repository owners should consider repository growth before scheduling large archives; this workflow is manual only.
